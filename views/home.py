"""
views/home.py — Página Home do sistema interno King Star.
Mantém todas as funcionalidades originais: esforço, pendências, agenda e perfil.
"""
...
def finalizar_atividade_atual(nome_usuario):
    ...
    for idx, act in enumerate(logs):
        if act["usuario"] == nome_usuario and act["status"] == "Em andamento":
            ...
            duracao                  = (agora - inicio_dt).total_seconds() / 60 if inicio_dt else 0
...
def exibir(user_info):
    st.markdown(
        ...
        .reminder-card, .diary-card {
            ...
            margin-bottom:12px; box-shadow:0 4px 6px -1px rgba(0,0,0,.1); border-left:6px solid;
        }
        .reminder-card { border-left-color:#ef4444; }
        .diary-card    { border-left-color:#3b82f6; }
        .status-tag    { font-size:.75rem; font-weight:800; text-transform:uppercase; }
        </style>
        """,
    ...
    tab_esforco, tab_pendentes, tab_agenda, tab_novo, tab_perfil = st.tabs([
        "⚡ Esforço Hoje", "🚀 Pendências", "📅 Agenda", "➕ Novo", "👤 Perfil"
    ])
                ...
                for l_idx in range(len(lembretes) - 1, -1, -1):
                    ...
                    try:
                        ...
                        if data_l_dt <= hoje_dt:
                            ...
                            st.markdown(
                                f"""<div class="reminder-card" style="border-left-color:{cor};">
    ...
    with tab_novo:
        ...
        with st.form("form_novo_v2"):
            ...
            d_agendada     = c1n.date_input("Para quando?", min_value=hoje_dt)
            h_agendada     = c2n.time_input("Qual horário?", value=datetime.now().time())

            proj_vinc = None
            if tipo == "Processos (PQI)":
                proj_vinc = st.selectbox("Selecione o Projeto PQI:", [p["titulo"] for p in projs] or ["(nenhum projeto)"])
